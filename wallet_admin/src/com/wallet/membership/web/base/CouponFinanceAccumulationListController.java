...
import com.wallet.membership.service.custom.CouponFinanceAccumulationService;

@Controller
public class CouponFinanceAccumulationListController extends CommonController{
	private final String PAGE_CODE = "COUPON_FINANCE_ACCUMULATION_LIST";
	 ...
	 * @since 2012.09.28
	 */
	@RequestMapping(value="/member/coupon_finance_accumulation_list.ms")
	public String CouponFinanceAccumulatonList(Map model, Locale locale, HttpServletRequest request, HttpServletResponse response) throws Exception{
		List<CouponFinanceAccumulation>  accumList = null;
		
		CouponFinanceAccumulationService ctaService = new CouponFinanceAccumulationService();
		...
		*/
		...
		params.put("compName", checkStr(request.getParameter("compName"), ""));//--금융사명
		params.put("name", checkStr(request.getParameter("name"), "")); //-- 쿠폰명
		
		log.debug("@@@@@@@@@@ CouponFinanceAccumulation params : "+ params); //##
		...
		accumList = ctaService.selectCouponFinanceAccumulationList(params); //-- 목록조회
		int accumListCnt = ctaService.selectCouponFinanceAccumulationListCnt(params);
		...
		params.clear();
		return "member/coupon_finance_accumulation_list";
	}
	
	/**
	 * @Method Name : CouponFinanceAccumulatonExcel
	 * @Description : 금융사별 정산 Excel
	 * @param : Map, Locale, HttpServletRequest, HttpServletResponse 
	 * @return : String 
	 * @author 김완섭
	 * @since 2012.09.28
	 */
	@RequestMapping(value="/member/coupon_finance_accumulation_list_excel.ms")
	public String CouponFinanceAccumulatonExcel(Map model, Locale locale, HttpServletRequest request, HttpServletResponse response) throws Exception{
		List<CouponFinanceAccumulation>  accumList = null;
		...
		*/
		...
		request.setAttribute("name", params.get("name"));
		params.clear();
		return "member/coupon_finance_accumulation_list_excel";
	}
